...
|--------|------|-------------|
| `GET` | `/.well-known/aip` | AIP gateway discovery for aipctl login (unauthenticated) |
...
| `POST` | `/agent-registrations/{name}/token` | Mint a session token for kubectl exec credential plugin (agent role) |
...
| `POST /agent-registrations/{name}/token` | `agent` |
| `GET /.well-known/aip` | unauthenticated |
